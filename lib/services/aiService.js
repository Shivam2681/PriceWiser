...
export async function generateAndStoreAIInsights(productId, forceRefresh = false) {
  try {
    ...
    const lastPrediction = normalizeDate(insights.pricePrediction?.lastUpdated);
    ...
    const hasFeatureSpecs =
      insights.features?.specifications &&
      Object.keys(insights.features.specifications).length > 0;
    if (forceRefresh || !hasFeatureSpecs) {
