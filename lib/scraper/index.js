...
export async function scrapeAmazonProduct(url) {
    ...
    try{
        ...
        // Validate: if current price is suspiciously low (less than 10% of original), 
        // it might be an EMI or accessory price - use original price instead
        ...
        if (currentPriceNum > 0 && originalPriceNum > 0) {
          if (currentPriceNum < (originalPriceNum * 0.1)) {
            console.log(`Price validation: Current price ₹${currentPriceNum} seems too low compared to original ₹${originalPriceNum}. Using original price.`);
