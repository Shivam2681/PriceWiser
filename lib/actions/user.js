...
function serializeProduct(product) {
  ...
  return {
    _id: plainProduct._id?.toString?.() || plainProduct._id,
    url: plainProduct.url,
    currency: plainProduct.currency,
    image: plainProduct.image,
    title: plainProduct.title,
    currentPrice: plainProduct.currentPrice,
    originalPrice: plainProduct.originalPrice,
    priceHistory: plainProduct.priceHistory?.map(ph => ({
      price: ph.price,
      date: ph.date?.toISOString?.() || new Date().toISOString()
    })) || [],
    discountRate: plainProduct.discountRate,
    category: plainProduct.category,
    reviewsCount: plainProduct.reviewsCount,
    stars: plainProduct.stars,
    isOutOfStock: plainProduct.isOutOfStock,
    // Serialize users array if it exists
    users: plainProduct.users?.map(u => ({
      email: u.email,
      _id: u._id?.toString?.() || u._id
    })) || [],
    lowestPrice: plainProduct.lowestPrice,
    highestPrice: plainProduct.highestPrice,
    averagePrice: plainProduct.averagePrice,
    addedAt: plainProduct.addedAt?.toISOString?.(),
    notifyEmail: plainProduct.notifyEmail,
...
export async function getUserStats(userId) {
  try {
    if (!isValidObjectId(userId)) return { trackedProducts: 0, favorites: 0, priceDrops: 0 };
...
export async function getUserTrackedProducts(userId) {
  try {
    ...
    return validTrackedProducts.map(tp => serializeProduct({
      ...tp.product.toObject(),
      addedAt: tp.addedAt,
...
export async function getUserFavorites(userId) {
  try {
    ...
    return validFavorites.map(serializeProduct);
  } catch (error) {
...
export async function isProductFavorited(userId, productId) {
  try {
    ...
    return user.favorites.some((fav) => fav.toString() === productId);
  } catch (error) {
