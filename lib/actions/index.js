...
import { generateAndStoreAIInsights } from "../services/aiService";
...
export async function scrapeAndStoreProduct(productUrl) {
    ...
    try {
      ...
      if(existingProduct) {
        ...
        // Set price stats - handle zero/invalid prices
        // If current price is 0 (pending), we can't set lowest/highest yet
        if (product.currentPrice && product.currentPrice > 0) {
          ...
          // Current price is 0/pending, initialize with 0 until valid price is scraped
          product.lowestPrice = 0;
          product.highestPrice = product.originalPrice || 0;
...
function serializeProduct(product) {
  ...
  // Use JSON stringify/parse to ensure all MongoDB types are converted to plain objects/primitives
  ...
  return {
    ...
    createdAt: product.createdAt?.toISOString?.() || plainProduct.createdAt,
    updatedAt: product.updatedAt?.toISOString?.() || plainProduct.updatedAt,
  };
}
...
export const getSimilarProducts = cache(async (productId) => {
    try {
      ...
      return similarProducts.map(serializeProduct);
    } catch (error) {
